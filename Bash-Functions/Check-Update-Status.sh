...
Check-Update-Status() {
  ...
  if [[ -z "$ProxyUrl" ]]; then
    ...
    status_cmd='curl_cli --max-time 20 -k --head "$1" 2>/dev/null | grep HTTP | awk '"'"'{print $2}'"'"''
  else
    echo "##############################################################################################################"
    echo "             ProxyUrl Provded: $ProxyUrl setting this to use for connectivty checks."
    echo "##############################################################################################################"
    # shellcheck disable=SC2016
    status_cmd='curl_cli --max-time 20 -k --proxy "$ProxyUrl" --head "$1" 2>/dev/null | grep HTTP | awk '"'"'{print $2}'"'"''
  fi
